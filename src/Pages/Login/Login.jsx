...
function Login() {
	...
	const submitForm = (values) => {
		try {
			...
			if (userFound) {
				const hashedInputPassword = CryptoJS.SHA256(values.password).toString(CryptoJS.enc.Hex);
					...
					{({ values, errors, touched, handleBlur, handleChange }) => (
						<Form className="space-y-4">
							<div className="flex flex-col relative">
								<Input icon={<EnvelopeIcon className="w-4 h-4 text-black" />} error={Boolean(errors.email && touched.email)} type="email" name="email" onChange={handleChange} onBlur={handleBlur} value={values.email} color="black" size="lg" variant="standard" autoComplete="off" label="Email" />
